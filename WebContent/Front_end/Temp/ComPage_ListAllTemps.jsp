			...
			<table class="table table-hover table-responsive" id="tempList">
				...
				<tbody>
					<c:forEach var="tempVO" items="${tempSvc.getAllByComNo(com_no)}"
						...
						<div class="modal fade" id="transferModal${s.count}" role="dialog">
							<div class="modal-dialog">
								...
								<div class="modal-content">
									...
									<div class="modal-footer">
										<button type="button" class="btn btn-default"
											data-dismiss="modal">取消</button>
										<button class="btn btn-danger" data-dismiss="modal"
											class='transferConfirm' onclick="document.getElementById('transfer${s.count}').submit();">確定</button>
									</div>
								</div>
						...
						<tr>
							...
							<td>
								<button class="btn btn-default" type='submit'
										name='displayTemp' onclick="javascript:location.href='<%=request.getContextPath()%>/Front_end/Temp/ComPage_ListAllTempConts.jsp?temp_no=${tempVO.temp_no}'">查看</button>
							...
							<td><button class='btn btn-default' data-toggle="modal"
										data-target="#transferModal${s.count}">匯入會員相簿</button>
