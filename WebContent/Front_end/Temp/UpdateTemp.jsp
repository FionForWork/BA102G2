		...
		<div class="row">
			<div>
				<form action="<%=request.getContextPath() %>/temp/temp.do" method="post">
					...
					<h4>
						<div class="input-group">
							<label for="create_date" class="input-group-addon">更改日期 </label>
					...
					<h4>
						<div class="input-group">
							<label class="input-group-addon" for="available">更改可挑選數量</label>
							<input type='number' name='available' id="available"
								value='${temp.available}' class="form-control">
						</div>
					</h4>
					<input type='hidden' name='action' value='update_Temp'> <input
						type='hidden' name='temp_no' value='temp_no'> <input
				...
				<form action="<%=request.getContextPath() %>/temp/temp.do" method="post">
					...
					<input type="submit" class='btn btn-default btn-block' value="刪除成品" />
				</form>
			</div>
		</div>
	</div>
					...
					<div class='dropdownContent' id='dropdownContent${s.count}'>
						...
						<form id="delete${s.count}"
							...
							<input type='hidden' name='tcont_no'
								...
								href='#'
								onclick="document.getElementById('delete${s.count}').submit();">刪除相片</a>
