	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("cropImage".equals(action)) {
			try {
				...
				forloop: for (AlbumVO album : albums) {
				...
				cropCont = contSvc.addContent(alb_no, new Timestamp(System.currentTimeMillis()), baos.toByteArray(),
						null);
		...
		if ("overlayImage".equals(action)) {
			...
			if (placeview_no.length() != 0) {
				...
				if (imageWidth.length() != 0) {
					...
					// double resizeHeight = (double)(originalimg.getHeight() /
					// ratio);
					...
					System.out.println("resizeHeight----" + resizeHeight);
					img = new BufferedImage(resizeWidth.intValue(), resizeHeight.intValue(),
				...
				if (getFileNameFromPart(part) != null && part.getContentType() != null) {
					...
					if (imageWidth.length() != 0) {
						...
						// System.out.println("ratio----" + ratio);
						// double resizeHeight =
						// (double)(originalimg.getHeight() / ratio);
			...
			if (cropWidth.length() != 0 && cropHeight.length() != 0) {
				...
				Image cropImageTmp = cropImage.getScaledInstance(intcropWidth, intcropHeight,
			...
			System.out.println("img.getWidth()" + img.getWidth());
			System.out.println("img.getHeight()" + img.getHeight());
			resultCont = contSvc.addContent(alb_no, new Timestamp(System.currentTimeMillis()), baos.toByteArray(),
					null);
