...
public class MemDAO implements MemDAO_Interface {
    ...
    private static final String INSERT_STMT  = "INSERT INTO member (mem_no,id,pwd,name,sex,bday,phone,email,account,picture,report,status) VALUES ('1'||ltrim(TO_CHAR(MEMID_SQ.NEXTVAL,'009')), ?, ?, ?, ?, ?, ?, ?, ?, null, 0,'停權')";
    private static final String GET_ALL_STMT = "SELECT mem_no,id,pwd,name,sex,to_char(bday,'yyyy-mm-dd') bday,phone,email,account,picture,report,status FROM member order by mem_no";
    private static final String GET_ONE_STMT = "SELECT mem_no,id,pwd,name,sex,to_char(bday,'yyyy-mm-dd') bday,phone,email,account,picture,report,status  FROM member where mem_no = ?";
    private static final String DELETE       = "DELETE FROM member where mem_no = ?";
    private static final String UPDATE       = "UPDATE member set id=?, pwd=?, name=?, sex=?, bday=?, phone=? ,email=?,account=? where mem_no = ?";
    private static final String UPDATEPIC    = "UPDATE member set picture=? where mem_no = ?";
